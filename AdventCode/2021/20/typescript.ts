...
async function padOutMap(map: string[][]): Promise<string[][]> {
  ...
  for (let i = 0; i < 2; i++) {
    let tmpA = new Array(map[0].length).fill('.')
    map.unshift(tmpA);
    map.push(tmpA);
  }
  return map
...
async function calculate(img: string[][], bounds: number): Promise<number> {
  let total = 0
  for (let i = 2; i < bounds-2; i++) {
    for (let j = 2; j < bounds-2; j++) {
      total += img[i][j] === "#" ? 1 : 0
    }
  }
  return total
